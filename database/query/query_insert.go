...
func (q *Query) InsertGetId(values any) (uint, error) {
	...
	if q.isPostgres() || q.isSQLServer() {
		...
		if tableName != "" {
			...
			if err != nil {
				// CURRVAL lookup failed (e.g. table uses GENERATED BY DEFAULT AS IDENTITY).
				// Fall back to MAX(id) as a last resort. The race window is acknowledged;
				// callers that need strict correctness should wrap InsertGetId in a transaction.
				...
				if q.tx != nil {
					...
					err = dbConn2.QueryRowContext(ctx, maxIDQuery).Scan(&lastID)
				}
				if err != nil {
