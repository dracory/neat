...
// User maps to the users.json table.
type User struct {
	ID     int    `db:"id"`
	Name   string `db:"name"`
...
// Product maps to the products.json table.
type Product struct {
	ID       int     `db:"id"`
	Name     string  `db:"name"`
...
// RunExample demonstrates the JSONDB driver: point at a directory of JSON, JSONL, or NDJSON
...
func RunExample() error {
	...
	// Create a JSONDB-driver database connection.
	// The directory is the database; each .json/.jsonl/.ndjson file becomes a table.
	config := neat.DBConfig{
		Default: "json_db",
		Connections: map[string]neat.ConnectionConfig{
			"json_db": {
				Driver: database.DriverJSONDB,
				Database: dataDir,
			},
		},
	}
	...
	// Query active users — data/users.json → "users" table
	...
	// Query expensive products — data/products.json → "products" table
	...
	// JOIN across two JSON-backed tables — orders (JSONL) + users (JSON)
