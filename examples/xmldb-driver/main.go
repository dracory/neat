...
// RunExample demonstrates the XMLDB driver: point at a directory of XML
...
func RunExample() error {
	...
	if _, err := os.Stat(dataDir); err != nil {
		...
		dataDir = filepath.Join("examples", "xmldb-driver", "data")
	}

	// Create an XMLDB-driver database connection.
	// The directory is the database; each .xml file becomes a table.
	config := neat.DBConfig{
		Default: "xml_db",
		Connections: map[string]neat.ConnectionConfig{
			"xml_db": {
				Driver: database.DriverXMLDB,
				Database: dataDir,
			},
		},
	}
	...
	// JOIN across two XML-backed tables — orders and users
