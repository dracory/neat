...
// This is the traditional soft delete approach using NULL to indicate active records
type Product struct {
...
// This example demonstrates NULL-based soft delete strategy
func main() {
	if err := RunExample("sqlite://./soft_delete_example.db"); err != nil {
		log.Fatalf("Example failed: %v", err)
	}
}

// RunExample demonstrates NULL-based soft delete usage
func RunExample(dsn string) error {
	...
	// Create products table with nullable soft_deleted_at column
	...
	err = db.Schema().Create("products", func(blueprint schema.Blueprint) {
		...
		// For NULL-based strategy, soft_deleted_at should be nullable with no default
	...
	fmt.Printf("Restored: %s ($%.2f) - SoftDeletedAt: %v\n",
		restoredProduct.Name,
		restoredProduct.Price,
		restoredProduct.SoftDeletedAt)
	...
	fmt.Println("\n=== Example Complete ===")
	fmt.Println("NULL-based soft delete strategy benefits:")
	fmt.Println("  - Simple and intuitive (NULL = active, non-NULL = deleted)")
	fmt.Println("  - Widely used pattern in many ORMs")
	fmt.Println("  - Easy to understand and debug")
