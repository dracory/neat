...
func TestTiDBTransactionCommit(t *testing.T) {
...
func TestTiDBTransactionRollback(t *testing.T) {
...
func TestTiDBTransactionErrorHandling(t *testing.T) {
...
func TestTiDBNestedTransactions(t *testing.T) {
...
func TestTiDBTransactionIsolationLevels(t *testing.T) {

	db := SetupTiDBTest(t)

	// TiDB does not support the SERIALIZABLE isolation level (it returns
	// Error 8048 unless tidb_skip_isolation_level_check=1 is set). TiDB
	// silently upgrades SERIALIZABLE to REPEATABLE-READ when that flag is
	// set, but by default it rejects it. Only test the supported levels here.
