...
// TestTiDBIntegrationRawUpdate tests raw SQL expressions in Update
func TestTiDBIntegrationRawUpdate(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}

	databaseConn := SetupTiDBTest(t)
...
// TestTiDBIntegrationRawWhere tests raw SQL expressions in Where clauses
func TestTiDBIntegrationRawWhere(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}
...
// TestTiDBIntegrationDatabaseFunctions tests database-specific raw functions
func TestTiDBIntegrationDatabaseFunctions(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}
	...
	// Update using TiDB-specific function (UPPER)
	_, err := query.Exec("UPDATE users SET avatar = UPPER(avatar) WHERE id = ?", createdUser.ID)
	if err != nil {
		t.Fatalf("Failed to update with TiDB function: %v", err)
	}
