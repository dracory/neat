...
func TestSQLServerSchemaColumnModifiers(t *testing.T) {
	if testing.Short() {
	...
	err := db.Schema().Create(tableName, func(table schema.Blueprint) {
		...
		table.String("collation_col").Collation("SQL_Latin1_General_CP1_CI_AS")
	...
	// SQL Server boolean is bit, default 1
	...
	if colMap["collation_col"].Collation != "SQL_Latin1_General_CP1_CI_AS" {
		t.Errorf("Expected collation 'SQL_Latin1_General_CP1_CI_AS', got '%s'", colMap["collation_col"].Collation)
	}
