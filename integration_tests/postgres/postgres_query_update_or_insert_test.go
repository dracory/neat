...
func TestPostgresIntegrationUpdateOrInsertWithStructInsert(t *testing.T) {
	...
	var user models.User
	err = db.Query().Table("users").Where("name = ?", "insert_struct").First(&user)
	if err != nil {
		t.Errorf("Failed to find inserted struct user: %v", err)
	}
	if user.Name != "insert_struct" {
		t.Errorf("Expected name 'insert_struct', got '%s'", user.Name)
	}
	if user.Avatar != "avatar_struct" {
		t.Errorf("Expected avatar 'avatar_struct', got '%s'", user.Avatar)
	}
}
...
func TestPostgresIntegrationUpdateOrInsertWithStructUpdate(t *testing.T) {
	...
	if user.Avatar != "avatar_struct_updated" {
		t.Errorf("Expected avatar 'avatar_struct_updated', got '%s'", user.Avatar)
	}
}
...
func TestPostgresIntegrationUpdateOrInsertWithWhereClause(t *testing.T) {
	...
	if *user.Bio != "bio_updated" {
		t.Errorf("Expected bio 'bio_updated', got '%s'", *user.Bio)
	}
}
