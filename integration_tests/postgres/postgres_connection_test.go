...
func TestPostgreSQLIntegrationConnection(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}

	config := GetPostgresConfig()
	config.Connections["postgres2"] = config.Connections["postgres"]
	...
	t.Run("Switch between connections", func(t *testing.T) {
		conn1 := db
		conn2, err := db.Connection("postgres2")
		if err != nil {
			t.Fatalf("Failed to get postgres2 connection: %v", err)
		}
	...
	t.Run("Default connection name", func(t *testing.T) {
		...
		if conn.Name() != "postgres" {
			t.Errorf("Expected connection name 'postgres', got '%s'", conn.Name())
		}
	})
...
func TestPostgreSQLIntegrationReadWriteSeparation(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}

	baseConn := GetPostgresConfig().Connections["postgres"]
...
func TestPostgreSQLIntegrationConnectionPoolSettings(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}
