...
func TestSQLiteSchemaTimestamp(t *testing.T) {
	if testing.Short() {
	...
	t.Run("DropTimestamps and DropSoftDeletes", func(t *testing.T) {
		...
		// SQLite has limitations for dropping columns (it might not be supported or requires table recreation)
		// Let's see if eloquent handles it.
		if err != nil {
	...
	t.Run("Precision", func(t *testing.T) {
		...
		// In SQLite, precision is often ignored in the type name but let's check what GetColumns returns
		columns, err := db.Schema().GetColumns(tableName)
		if err != nil {
			t.Fatalf("Failed to get columns: %v", err)
		}
