...
func TestSQLiteSchemaTimestamps(t *testing.T) {
	if testing.Short() {
	...
	for _, col := range columns {
		if col.Name == "created_at" {
			...
			if col.TypeName != "datetime" {
				t.Errorf("Expected datetime type for created_at, got %s", col.TypeName)
			}
			if !col.Nullable {
		...
		if col.Name == "updated_at" {
			hasUpdatedAt = true
			if col.TypeName != "datetime" {
				t.Errorf("Expected datetime type for updated_at, got %s", col.TypeName)
			}
			if !col.Nullable {
...
func TestSQLiteSchemaTimestampsTz(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}
	...
	if err != nil {
		t.Fatalf("Failed to create tz table: %v", err)
	}

	columns, err := db.Schema().GetColumns(tableNameTz)
	if err != nil {
		t.Fatalf("Failed to get tz columns: %v", err)
	}
	...
	if !hasCreatedAt {
		t.Error("created_at column not found in tz table")
	}
	if !hasUpdatedAt {
		t.Error("updated_at column not found in tz table")
	}

	_ = db.Schema().Drop(tableNameTz)
}

func TestSQLiteSchemaSoftDeletes(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}
	...
	for _, col := range columns {
		if col.Name == constants.SoftDeleteAtColumn {
			hasDeletedAt = true
			if col.TypeName != "datetime" {
				t.Errorf("Expected datetime type for soft_deleted_at, got %s", col.TypeName)
			}
			if !col.Nullable {
		...
		if col.Name == "deleted_at_tz" {
			hasDeletedAtTz = true
			if col.TypeName != "datetime" {
				t.Errorf("Expected datetime type for deleted_at_tz, got %s", col.TypeName)
			}
			if !col.Nullable {
	...
	if !hasDeletedAt {
		t.Error("soft_deleted_at column not found")
	}
	if !hasDeletedAtTz {
		t.Error("deleted_at_tz column not found")
	}

	_ = db.Schema().Drop(tableName)
}

func TestSQLiteSchemaDropTimestamps(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}
...
func TestSQLiteSchemaDropTimestampsTz(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}
	...
	if err != nil {
		t.Fatalf("Failed to drop tz timestamps/soft deletes: %v", err)
	}

	if db.Schema().HasColumn(tableNameTz, "created_at") {
		t.Error("created_at should not exist after tz drop")
	}
	if db.Schema().HasColumn(tableNameTz, "updated_at") {
		t.Error("updated_at should not exist after tz drop")
	}
	if db.Schema().HasColumn(tableNameTz, "deleted_at_tz") {
		t.Error("deleted_at_tz should not exist after tz drop")
	}

	_ = db.Schema().Drop(tableNameTz)
}

func TestSQLiteSchemaTimestampPrecision(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}
	...
	for _, col := range columns {
		if col.Name == "created_at" || col.Name == "updated_at" || col.Name == constants.SoftDeleteAtColumn {
			if col.TypeName != "datetime" {
				t.Errorf("Expected datetime type for %s, got %s", col.Name, col.TypeName)
			}
		}
	}
