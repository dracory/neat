...
func TestSQLiteIntegrationRestore(t *testing.T) {
	...
	res, err = db.Query().Model(&models.User{}).WithSoftDeleted().Where("name = ?", "restore_user3").RestoreSoftDeleted()
	if err != nil {
	...
	res, err = db.Query().Model(&models.User{}).WithSoftDeleted().Where("name = ?", "restore_user4").RestoreSoftDeleted()
	if err != nil {
