...
func TestSQLiteSchemaTable(t *testing.T) {
	if testing.Short() {
	...
	t.Run("DropAllTables", func(t *testing.T) {
		// Use a separate in-memory DB for this to avoid affecting other tests if they don't clean up
	...
	t.Run("Table Prefix", func(t *testing.T) {
		...
		// HasTable should work with the logical name
		...
		// GetTableListing should return logical names (Wait, does it? Goravel's behavior usually hides prefix in some places and shows in others)
		// Actually, in many implementations, GetTableListing returns the names as they are in the database.
		// Let's see what eloquent does.
