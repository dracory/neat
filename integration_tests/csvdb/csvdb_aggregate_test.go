...
func TestCSVDBIntegrationCountAll(t *testing.T) {
	db := SetupCSVDBTest(t)

	var result []csvdbCountResult
	err := db.Query().
		Table("users").
		Select("COUNT(*) AS cnt").
		Get(&result)
...
func TestCSVDBIntegrationCountWithWhere(t *testing.T) {
	db := SetupCSVDBTest(t)
...
func TestCSVDBIntegrationSum(t *testing.T) {
	db := SetupCSVDBTest(t)

	var result []csvdbSumResult
	err := db.Query().
		Table("orders").
...
func TestCSVDBIntegrationAvg(t *testing.T) {
	db := SetupCSVDBTest(t)

	var result []csvdbAvgResult
	err := db.Query().
		Table("products").
...
func TestCSVDBIntegrationMinMax(t *testing.T) {
	db := SetupCSVDBTest(t)

	var result []csvdbMinMaxResult
	err := db.Query().
		Table("products").
...
func TestCSVDBIntegrationGroupBy(t *testing.T) {
	db := SetupCSVDBTest(t)
	...
	if results[0].Count != 2 {
		t.Errorf("expected Electronics count 2, got %d", results[0].Count)
	}
...
func TestCSVDBIntegrationSumWithJoin(t *testing.T) {
	db := SetupCSVDBTest(t)
