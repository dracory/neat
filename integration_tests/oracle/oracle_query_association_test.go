...
func TestOracleIntegrationQueryPolymorphicAssociation(t *testing.T) {
	...
	// POSTS, VIDEOS, and COMMENTS tables are now created in helper.go
	// Cleanup test data before running the test
	_, _ = query.Exec("TRUNCATE TABLE POSTS CASCADE")
	_, _ = query.Exec("TRUNCATE TABLE VIDEOS CASCADE")
	_, _ = query.Exec("TRUNCATE TABLE COMMENTS CASCADE")
