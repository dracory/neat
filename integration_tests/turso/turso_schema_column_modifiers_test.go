...
func TestTursoSchemaColumnModifiers(t *testing.T) {
	if testing.Short() {
	...
	err := db.Schema().Create(tableName, func(table schema.Blueprint) {
		...
		// Collation is supported by Turso (SQLite)
		table.String("collation_col").Collation("NOCASE")
		// Comments are not supported by Turso (SQLite) grammar (returns empty string), but shouldn't error
	...
	// Turso (SQLite) doesn't have a separate unsigned type for integers, usually they are just 'integer'
	// But it should have been accepted.

	// Check collation - Turso (SQLite) processor currently doesn't populate collation from DB
	...
	err = db.Schema().Table(tableName, func(table schema.Blueprint) {
		// Turso (SQLite) doesn't support ALTER COLUMN, so this should be a no-op or handled by recreation
	...
	// Test After and First - should be ignored by Turso without error
	err = db.Schema().Table(tableName, func(table schema.Blueprint) {
...
func TestTursoSchemaColumnModifiersExpression(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}
