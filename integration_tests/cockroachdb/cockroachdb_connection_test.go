...
func TestCockroachDBIntegrationConnectionSwitch(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}

	config := GetCockroachDBConfig()
	config.Connections["cockroachdb2"] = config.Connections["cockroachdb"]
	...
	conn1 := db
	conn2, err := db.Connection("cockroachdb2")
	if err != nil {
		t.Fatalf("Failed to get cockroachdb2 connection: %v", err)
	}
...
func TestCockroachDBIntegrationConnectionDefaultName(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}
...
func TestCockroachDBIntegrationReadWriteSeparation(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}

	baseConn := GetCockroachDBConfig().Connections["cockroachdb"]
	...
	config := neat.DBConfig{
		...
		Connections: map[string]neat.ConnectionConfig{
			"rw": {
				...
				SSLMode: baseConn.SSLMode,
				Schema:  baseConn.Schema,
			},
		},
	}
...
func TestCockroachDBIntegrationConnectionPoolSettings(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}
