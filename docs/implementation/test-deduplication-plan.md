...
**Action:** Create `paginate_helpers.go` with shared functions
...
**Action:** Create `create_helpers.go` with shared functions, keep BigSerial tests in postgres package
...
**Action:** Create `delete_helpers.go` with shared functions (harmonize nil checks or make them optional)
...
**Action:** Create `join_helpers.go` with shared seed function and test functions
...
**Action:** Create `group_having_helpers.go` with shared seed function and test functions
...
3. **Delete Tests** - Identical logic, SQLite has extra nil checks (can harmonize)
...
10. **Aggregate Tests** - Different seeding strategies (time fields vs explicit IDs)
